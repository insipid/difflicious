    ...
    with app.test_client() as client:
        ...
        if response.status_code == 200:
            ...
            if diffs:
                ...
                for i, hunk in enumerate(claude_file['hunks'][:2]):  # Show first 2 hunks
                    print(f"\n   🔸 Hunk {i+1}:")
                    print(f"      Range: {hunk['old_start']}-{hunk['old_start']+hunk['old_count']-1} -> {hunk['new_start']}-{hunk['new_start']+hunk['new_count']-1}")
                    ...
                    for j, line in enumerate(hunk['lines'][:3]):  # Show first 3 lines
                        ...
                        if line['type'] == 'context':
                            left = line['left']
                            print(f"           Context {left['line_num']}: {left['content'][:50]}...")
                            
                        elif line['type'] == 'change':
                            left = line['left']
                            right = line['right']
                            print(f"           Left  {left.get('line_num', '---'):3}: {left.get('content', '')[:40]}...")
                            print(f"           Right {right.get('line_num', '---'):3}: {right.get('content', '')[:40]}...")
                            print(f"           Types: {left.get('type', 'none')} -> {right.get('type', 'none')}")
                    
                    if len(hunk['lines']) > 3:
                        print(f"        ... ({len(hunk['lines'])-3} more lines)")
                
                print(f"\n💾 Full response structure available for frontend!")
                print(f"✅ Side-by-side diff data ready for rendering!")
