...
- [ ] **C1. Version still at 0.10.0**
  Deferred to final release commit — version bump will be the last change before tagging v1.0.0.
...
- [x] **C5. pyproject.toml classifier says "Beta"** ✅ FIXED
  Changed to `"Development Status :: 5 - Production/Stable"`.

- [x] **C6. Placeholder author email** ✅ FIXED
  Removed `noreply@example.com` from both `__init__.py` and `pyproject.toml`.

### Codebase: High Priority

- [x] **H1. `cilicious.sh` modifies files instead of checking** ✅ FIXED
  Added `#!/bin/bash` shebang, `set -e`, and changed `black .` to `black --check .`.
...
- [x] **H3. `diff_parser.py:14-41` uses `subprocess.run(["wc", "-l", ...])`** ✅
  Replaced with pure Python line counting. More portable (no shell dependency) and safer (no subprocess).
...
- [x] **M1. Integration tests aren't real pytest tests** ✅ FIXED (Batch 9)
  Converted all three integration test files to proper `def test_*()` pytest functions using the `client` fixture. No `def main()` or `sys.path.insert` remain.
...
- [x] **M3. Unused parameter in `template_service.py:40`** ✅
  Removed `target_commit` parameter (was part of M2 cleanup).

- [x] **M4. Inconsistent error HTTP status codes** ✅
  Updated all API error responses to use `error_response()` helper and return proper HTTP 500 status codes.

- [x] **M5. ruff/black line-length mismatch** ✅
  Aligned ruff line-length to 88 (Black's default). Added E501 to ruff ignore list since Black handles line length where possible, and some lines (comments, strings) can't be wrapped.

- [x] **M6. No tests for auto-reload routes/SSE** ✅ FIXED (Batch 9)
  Added `tests/test_auto_reload_routes.py` with dedicated coverage for `auto_reload_routes.py`.

- [x] **M7. No test for `diff_parser._get_file_line_count`** ✅ FIXED (Batch 9)
  Added `tests/unit/test_diff_parser_line_count.py` with direct tests for the function.

- [x] **M8. Limited JS test coverage** ✅ FIXED (Batch 9)
  Added 10 new JS test files: `context-expansion.test.js`, `context-expansion-ui.test.js`, `full-diff.test.js`, `auto-reload.test.js`, `stores/diffStore.test.js`, `stores/searchStore.test.js`, `stores/themeStore.test.js`, `components/fileComponent.test.js`, `components/groupComponent.test.js`, `components/hunkComponent.test.js`. Now 14 test suites, 133 JS tests.

- [x] **M9. Dual dev dependency specs in pyproject.toml** ✅ FIXED
  Removed stale `[project.optional-dependencies] dev` group with old version pins. Only `[dependency-groups] dev` with current versions remains.

- [x] **M10. `git_operations.py` mislabeled in docs** ✅ FIXED (Batch 7)
  Removed "Legacy / being phased out" label from CLAUDE.md. Now correctly described as core git utilities used by the service layer.

- [x] **M11. Unvalidated commit refs in `git_operations.py:790-798`** ✅
  Added `_is_safe_commit_sha()` validation for `base_commit` and `target_commit` in `_get_file_diff()`.
...
- [x] **D1. README.md:45 — Wrong Docker image name** ✅
  Fixed to `insipid/difflicious:latest`.
...
- [x] **D4. README.md:18 — Wrong syntax highlighting library** ✅
  Fixed to "Pygments" and "100+ languages".
...
- [x] **D6. README.md:152-157 — SSE listed as future, but it's implemented** ✅
  Moved SSE from "Future Enhancements" to "Completed" section.
...
- [x] **D12. README.md:119 — Says "vanilla CSS" but project uses Tailwind** ✅
  Fixed to "Alpine.js + Tailwind CSS".

- [x] **D13. README.md:161 — Contributing section says "early development"** ✅
  Rewrote to link to CONTRIBUTING.md with summary of contents.

- [x] **D14. README.md — No badges or screenshots** ✅
  Added PyPI, Python version, and license badges. Added TODO comment for screenshots.

- [x] **D15. README.md:147 — Stale test count/coverage numbers** ✅
  Updated to "135+ tests, 70%+ coverage".

- [x] **D16. INSTALLATION.md:262 — Phantom `PORT` env var** ✅
  Now implemented as `DIFFLICIOUS_PORT` and `DIFFLICIOUS_HOST` environment variables with `-p`/`-h` CLI shortcuts.
...
### Batch 1: Release Metadata & Config ✅ COMPLETED (except C1)

- [ ] Bump `__version__` to `1.0.0` — deferred to final release commit (C1)
- [x] Update pyproject.toml classifier to `Production/Stable` (C5) ✅
- [x] Fix placeholder email in `__init__.py` and `pyproject.toml` (C6) ✅
- [x] Reconcile dual dev dependency groups in pyproject.toml (M9/D10) ✅
- [x] Add missing classifiers and URLs to pyproject.toml (D22) ✅
- [x] Fix `cilicious.sh`: add shebang, `set -e`, use `black --check .` (H1) ✅

**Branch:** `batch1-release-metadata-v1.0` / `fix-cilicious-v1.0`
...
### Batch 4: Backend Cleanup ✅ COMPLETED

- [x] Replace `wc -l` subprocess in `diff_parser.py` with Python (H3)
- [x] Remove dead parameters from `diff_service.py` (M2)
- [x] Remove unused `target_commit` param from `template_service.py` (M3)
- [x] Harmonize ruff/black line-length or document why they differ (M5)
- [x] Fix inconsistent HTTP status codes in error responses (M4)
- [x] Validate commit refs in `git_operations.py` (M11)
- [x] Add `DIFFLICIOUS_PORT` and `DIFFLICIOUS_HOST` environment variables (bonus)

**Branch:** `backend-cleanup-v1.0` — PR #52
**Commits:** 6 commits for individual changes

### Batch 5: README.md Fixes ✅ COMPLETED

- [x] Fix Docker image name (D1)
- [x] Fix Highlight.js to Pygments, 30+ to 100+ (D4)
- [x] Move SSE from "Future" to implemented features (D6)
- [x] Fix "vanilla CSS" to "Tailwind CSS" (D12)
- [x] Rewrite contributing section, link to CONTRIBUTING.md (D13)
- [x] Add badges (PyPI, Python version, license, CI) (D14)
- [x] Add screenshot placeholder for light/dark mode (D14)
- [x] Update test count/coverage numbers to 135+/70%+ (D15)

**Branch:** `readme-fixes-v1.0`
**Commits:** 8 commits for individual changes

### Batch 6: Other Documentation Fixes

- [x] Fix CHANGELOG dates for 0.9.1/0.9.2 (D3)
- [x] Draft CHANGELOG 1.0.0 entry (D5)
- [x] Remove SSE from "Under Consideration" in CHANGELOG (D7)
- [x] Remove duplicate release notes section from CHANGELOG (D23)
- [x] Update "Planned for 1.0" in CHANGELOG (D24)
- [x] Remove or implement `DIFFLICIOUS_REPO_PATH` in INSTALLATION.md (D2)
- [x] Verify or remove `PORT` env var in INSTALLATION.md (D16) — implemented as `DIFFLICIOUS_PORT`
- [x] Reduce duplication between INSTALLATION.md and README (D17)
- [x] Fix `npm` to `pnpm` in CONTRIBUTING.md (D9)
- [x] Add `cilicious.sh` to CONTRIBUTING.md workflow (D11)
- [x] Align coverage targets across docs (D19)

**Branch:** `batch6-doc-fixes-v1.0`

### Batch 7: CLAUDE.md & Internal Docs

- [x] Update project structure tree (D8)
- [x] Remove "being phased out" label from `git_operations.py` (M10)
- [x] Add `watchdog` to Key Dependencies (D26)
- [x] Add version reference header (D30)
- [x] Update CONTRIBUTING.md project structure tree (D18)
- [x] Update javascript-modularization-plan.md status (D20)
- [x] Clarify Alpine component architecture doc status (D21)
- [x] Mark PLAN.md as historical (D25)

**Branch:** `batch7-internal-docs-v1.0`

### Batch 8: Polish & Low Priority

- [x] Fix bare `dict` return types in `config.py`, `app.py` (L2, L3)
- [x] Consider upper bounds on runtime dependencies (L4)
- [x] Move `twine` to separate dependency group (L5)
- [x] Use section names instead of line numbers in CSS guide (D27)
- [x] Organize `docs/` internal artifacts (D28)
- [x] Delete or `.gitignore` `TODO.org` (D29)

**Branch:** `batch8-polish-v1.0`

### Batch 9: Test Improvements (Post-v1.0 OK)

- [x] Convert or relocate integration test scripts (M1)
- [x] Add tests for auto-reload routes/SSE (M6)
- [x] Add test for `_get_file_line_count` (M7)
- [x] Expand JS test coverage (M8)

**Branch:** `batch9-tests-v1.0`
