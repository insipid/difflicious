...
**Target Release:** 1.1.0 (1.0.0 was published to PyPI accidentally; skipping it)
...
- [ ] **C1. Version still at 0.10.0**
  `src/difflicious/__init__.py:3` — `__version__ = "0.10.0"` needs to become `"1.1.0"`.
  **Note:** 1.0.0 was accidentally published to PyPI during a pipeline test; target is 1.1.0.
...
- [x] **D5. CHANGELOG.md — No release entry** ✅ FIXED
  Added `## [1.1.0]` entry (1.0.0 skipped — accidentally published to PyPI).
...
- [ ] Bump `__version__` to `1.1.0` — `__init__.py` (C1) — deferred to final release commit
...
- [x] Draft CHANGELOG 1.1.0 entry (D5) ✅ — 1.0.0 skipped, see above
