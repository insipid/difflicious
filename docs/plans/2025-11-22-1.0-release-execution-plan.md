...
- `test_api_detailed.py` → `tests/integration/test_api_detailed.py`
- `test_frontend.py` → `tests/integration/test_frontend.py`
- `test_parser.py` → `tests/unit/test_parser.py`
- `test_side_by_side.py` → `tests/unit/test_diff_parser_side_by_side.py`
- `test_rendering_parser.py` → `tests/unit/test_diff_rendering.py`
...
**Specific Issues:**
1. **PLAN.md** shows completed features as "⏸️" (paused)
...
2. Change all "⏸️" to "✅" for completed Phase 1 items
3. Update "Current Status" section to reflect 0.9.2 → 1.0 transition
...
4. Update `docs/reports/2025-11-05-2019-color-audit.md` or create new audit
...
**Current Behavior:**
- User runs `difflicious` outside git repo → unclear error or crash
...
1. ✅ **All MUST FIX items complete** - No blockers remain
2. ✅ **Theme refinements complete** - Both light and dark modes are pleasant
3. ✅ **All tests passing** - cilicious.sh runs clean
4. ✅ **Documentation accurate** - README, PLAN, CLAUDE in sync
5. ✅ **Version bumped** - 1.0.0 in all files
6. ✅ **CHANGELOG updated** - Complete release notes
7. ✅ **No FIXME/TODO comments** - Production-ready code
8. ✅ **Metadata updated** - pyproject.toml reflects stable status
